...
def _():
    ...
    return duckdb, json, mo, os, pathlib_path, pd, plt, sns, tomllib
...
@app.cell
def _(mo, os, pathlib_path, pd, tomllib):
    """Read the active runtime configuration from config.toml."""
    config_path = pathlib_path(os.getenv("CONFIG_FILE", "config.toml"))
...
@app.cell
def _(checkpoint_cfg, json, mo, pathlib_path, pd):
    """Inspect checkpoint progress files used for resume and recovery."""
    checkpoint_root = pathlib_path(checkpoint_cfg.get("base_dir", "checkpoints"))
...
@app.cell
def _(mo, pathlib_path, results_cfg):
    """Describe where final experiment results are stored and how reruns behave."""
    runs_root = pathlib_path(results_cfg.get("out_dir", "./runs"))
